#include "../include/config.h"

const double KB = 1.380649e-23; // Boltzmann constant in J/K
const double NA = 6.02214076e23; // Avogadro's number in 1/mol

// size of domain (m)
const double Lx = 1.0;
const double Ly = 1.0;
const double Lz = 1.0;
const double DL = 0.1; // reservoir thickness for boundary conditions

// gas properties
const double molarMass = 0.040;
// hard sphere assumption
const double diameter = 4.0e-10;

// initial velocity (m/s)
const double ux0 = 0.0;
const double uy0 = 0.0;
const double uz0 = 0.0;

// simulation loop parameters
const double dt = 1.0e-5;
const int nSteps = 2000;
const int printPeriod = 200;
const int particlesPerCellTarget = 200;
const int firstSampleStep = 1000;
const int samplingPeriod = 10;

// boundary conditions
double Tleft = 400.0;
double Tright = 200.0;

// wing
double WingX = 0.3; // X coordinate of the wing leading edge (m)
double WingY = 0.5; // Y coordinate of the wing leading edge (m)
double WingLength = 0.1; // Length of the wing (m)
double Tw = 300.0; // Temperature of the wing surface (K

// stream
double MaFree = 4.0; // Free stream Mach number
double PFree = 0.1; // Free stream pressure (Pa)
double TFree = 200.0; // Free stream temperature (K)
double angleOfAttack = 30.0; // Angle of attack (degrees)